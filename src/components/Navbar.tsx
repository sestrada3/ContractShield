...
export default function Navbar() {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
      scrolled ? 'bg-dark-950/95 backdrop-blur-md border-b border-dark-600/50 shadow-lg' : 'bg-transparent'
    }`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center gap-8">
            {navLinks.map((link) => (
              <a key={link.href} href={link.href} className="text-white/55 hover:text-white text-sm font-medium transition-colors">{link.label}</a>
            ))}
          </div>
      ...
      {isOpen && (
        <div className="md:hidden bg-dark-900/98 backdrop-blur-md border-b border-dark-600/30">
